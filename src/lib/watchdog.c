/* watchdog.c generated by valac 0.14.2, the Vala compiler
...
static gchar* fso_gsm_generic_watch_dog_real_repr (FsoFrameworkAbstractObject* base) {
	...
	pin = _tmp3_;
	_tmp4_ = data->keepRegistration;
	keep = _tmp4_;
	_tmp5_ = string_to_string (pin);
	_tmp6_ = bool_to_string (keep);
	_tmp7_ = _tmp6_;
	_tmp8_ = g_strconcat ("<P:", _tmp5_, "|KR:", _tmp7_, ">", NULL);
	_tmp9_ = _tmp8_;
	_g_free0 (_tmp7_);
	result = _tmp9_;
	_g_free0 (pin);
...
static gboolean fso_gsm_generic_watch_dog_unlockModem_co (fso_gsm_generic_watch_dog_unlockModemData* _data_) {
	...
	__catch61_g_error:
	{
		...
		{
			_data_->_state_ = 2;
			fso_gsm_gatherSimStatusAndUpdate (fso_gsm_generic_watch_dog_unlockModem_ready, _data_);
			return FALSE;
			_state_2:
		...
		__catch62_g_error:
		{
			...
			_data_->_tmp17_ = NULL;
			_data_->_tmp17_ = g_strconcat ("Can't gather SIM status: ", _data_->_tmp16_, NULL);
...
FsoGsmGenericWatchDog* fso_gsm_generic_watch_dog_construct (GType object_type) {
	...
	self->priv->lastStatus = _tmp1_;
	_tmp2_ = fso_gsm_theModem;
	g_signal_connect_object (_tmp2_, "signal-status-changed", (GCallback) _fso_gsm_generic_watch_dog_onModemStatusChange_fso_gsm_modem_signal_status_changed, self, 0);
	return self;
}


FsoGsmGenericWatchDog* fso_gsm_generic_watch_dog_new (void) {
	return fso_gsm_generic_watch_dog_construct (FSO_GSM_TYPE_GENERIC_WATCH_DOG);
}
