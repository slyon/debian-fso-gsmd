/* smsstorage.c generated by valac 0.16.1, the Vala compiler
...
static GVariant* _variant_new72 (gint value);
static GVariant* _variant_new73 (gint value);
static GVariant* _variant_new74 (gboolean value);
static GVariant* _variant_new75 (gboolean value);
static GVariant* _variant_new76 (gboolean value);
static GVariant* _variant_new77 (gboolean value);
static GVariant* _variant_new78 (gboolean value);
static GVariant* _variant_new79 (guint8 value);
static GVariant* _variant_new80 (guint8 value);
static GVariant* _variant_new81 (guint8 value);
static GVariant* _variant_new82 (gchar* value);
static GVariant* _variant_new83 (gchar* value);
static FreeSmartphoneGSMSIMMessage* fso_gsm_sms_storage_real_messagebook (FsoGsmISmsStorage* base, int* result_length1);
static void _vala_array_add29 (FreeSmartphoneGSMSIMMessage** array, int* length, int* size, const FreeSmartphoneGSMSIMMessage* value);
...
static void sms_deliver_addProperties (struct sms_deliver *self, GHashTable* props) {
	...
	if (_tmp33_) {
		...
		_tmp44_ = _tmp43_;
		_tmp45_ = _variant_new83 (_tmp44_);
...
static GHashTable* sms_properties (struct sms* self) {
	...
	if (_tmp7_) {
		...
		_tmp10_ = src;
		_tmp11_ = _variant_new72 (_tmp10_);
		g_hash_table_insert (_tmp8_, _tmp9_, _tmp11_);
		_tmp12_ = props;
		_tmp13_ = g_strdup ("app-port-dst");
		_tmp14_ = dst;
		_tmp15_ = _variant_new73 (_tmp14_);
		g_hash_table_insert (_tmp12_, _tmp13_, _tmp15_);
		_tmp16_ = props;
		_tmp17_ = g_strdup ("app-port-8bit");
		_tmp18_ = is8bit;
		_tmp19_ = _variant_new74 (_tmp18_);
		g_hash_table_insert (_tmp16_, _tmp17_, _tmp19_);
	}
	_tmp20_ = self->type;
	switch (_tmp20_) {
...
static FreeSmartphoneGSMSIMMessage* fso_gsm_sms_storage_real_messagebook (FsoGsmISmsStorage* base, int* result_length1) {
	...
	index = 0;
	{
		...
		while (TRUE) {
			...
			_vala_array_add29 (&mb, &mb_length1, &_mb_size_, &_tmp15_);
			_g_free0 (key);
		}
