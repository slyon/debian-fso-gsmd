/* atparser.c generated by valac 0.14.2, the Vala compiler
	...
	if (_tmp2_) {
		...
		switch (_tmp3_) {
			...
			default:
			{
				...
				_tmp5_ = c;
				_vala_array_add6 (&self->priv->curline, &self->priv->curline_length1, &self->priv->_curline_size_, _tmp5_);
				result = FSO_GSM_STATE_BASED_AT_PARSER_STATE_INLINE;
				return result;
			}
		}
		...
		_tmp7_ = c;
		_vala_array_add7 (&self->priv->curline, &self->priv->curline_length1, &self->priv->_curline_size_, _tmp7_);
		result = FSO_GSM_STATE_BASED_AT_PARSER_STATE_INLINE;
		return result;
	}
}
...
FsoGsmStateBasedAtParserState fso_gsm_state_based_at_parser_v0_result (FsoGsmStateBasedAtParser* self, gchar c) {
	...
	switch (_tmp0_) {
		case '\r':
		{
			...
			_vala_array_add8 (&self->priv->curline, &self->priv->curline_length1, &self->priv->_curline_size_, 'O');
			_tmp2_ = self->priv->curline;
			_tmp2__length1 = self->priv->curline_length1;
			_vala_array_add9 (&self->priv->curline, &self->priv->curline_length1, &self->priv->_curline_size_, 'K');
...
FsoGsmStateBasedAtParserState fso_gsm_state_based_at_parser_inline (FsoGsmStateBasedAtParser* self, gchar c) {
	...
	switch (_tmp0_) {
		...
		default:
		{
			...
			_tmp1__length1 = self->priv->curline_length1;
			_tmp2_ = c;
			_vala_array_add10 (&self->priv->curline, &self->priv->curline_length1, &self->priv->_curline_size_, _tmp2_);
			result = FSO_GSM_STATE_BASED_AT_PARSER_STATE_INLINE;
			return result;
		}
	}
...
FsoGsmStateBasedAtParserState fso_gsm_state_based_at_parser_endoflinePerhapsSolicited (FsoGsmStateBasedAtParser* self) {
	...
	if (_tmp4_) {
		...
		_vala_array_add12 (&self->priv->solicited, &self->priv->solicited_length1, &self->priv->_solicited_size_, _tmp7_);
	...
	_vala_array_add13 (&self->priv->solicited, &self->priv->solicited_length1, &self->priv->_solicited_size_, _tmp16_);
