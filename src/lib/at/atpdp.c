/* atpdp.c generated by valac 0.14.2, the Vala compiler
...
struct _FsoGsmAtPdpHandlerScActivateData {
	...
	GError* _tmp52_;
	FsoFrameworkGProcessGuard* _tmp53_;
...
static gchar** fso_gsm_at_pdp_handler_real_buildCommandLine (FsoGsmAtPdpHandler* self, int* result_length1) {
	...
	data = _tmp1_;
	_tmp2_ = data;
	_tmp3_ = _tmp2_->pppCommand;
	_tmp4_ = g_strdup (_tmp3_);
	_tmp5_ = fso_gsm_theModem;
	_tmp6_ = fso_gsm_modem_allocateDataPort (_tmp5_);
	...
	_tmp8_ = data;
	_tmp9_ = _tmp8_->pppOptions;
	_tmp9__length1 = _tmp8_->pppOptions_length1;
	{
		gchar** option_collection = NULL;
		gint option_collection_length1 = 0;
		gint _option_collection_size_ = 0;
		gint option_it = 0;
		option_collection = _tmp9_;
		option_collection_length1 = _tmp9__length1;
		for (option_it = 0; option_it < _tmp9__length1; option_it = option_it + 1) {
			gchar* _tmp10_;
			gchar* option = NULL;
			_tmp10_ = g_strdup (option_collection[option_it]);
			option = _tmp10_;
			{
				gchar** _tmp11_;
				...
				gchar* _tmp13_;
				_tmp11_ = cmdline;
				_tmp11__length1 = cmdline_length1;
				_tmp12_ = option;
				_tmp13_ = g_strdup (_tmp12_);
				_vala_array_add16 (&cmdline, &cmdline_length1, &_cmdline_size_, _tmp13_);
				_g_free0 (option);
			}
		}
	}
	_tmp14_ = cmdline;
	_tmp14__length1 = cmdline_length1;
	if (result_length1) {
...
static gboolean fso_gsm_at_pdp_handler_real_enterDataState_co (fso_gsm_at_pdp_handler_enterDataStateData* _data_) {
	...
	_data_->_tmp1_ = NULL;
	_data_->_tmp1_ = fso_gsm_modem_createAtCommand (_data_->_tmp0_, FSO_GSM_TYPE_V250_D, (GBoxedCopyFunc) g_object_ref, g_object_unref, "D");
	_data_->cmd = (FsoGsmV250D*) _data_->_tmp1_;
	_data_->_tmp2_ = fso_gsm_theModem;
	_data_->_tmp3_ = NULL;
	_data_->_tmp3_ = fso_gsm_v250_d_issue (_data_->cmd, "*99***1#", FALSE);
	_data_->_tmp4_ = _data_->_tmp3_;
	_data_->_tmp5_ = 0;
	_data_->_state_ = 1;
	fso_gsm_modem_processAtCommandAsync (_data_->_tmp2_, (FsoGsmAtCommand*) _data_->cmd, _data_->_tmp4_, FSO_GSM_MODEM_DEFAULT_RETRIES, fso_gsm_at_pdp_handler_enterDataState_ready, _data_);
	return FALSE;
	_state_1:
	_data_->_tmp6_ = NULL;
...
static gboolean fso_gsm_at_pdp_handler_real_leaveDataState_co (fso_gsm_at_pdp_handler_leaveDataStateData* _data_) {
	...
	_data_->_tmp0_ = fso_gsm_theModem;
	_data_->_tmp1_ = NULL;
	_data_->_tmp1_ = fso_gsm_modem_createAtCommand (_data_->_tmp0_, FSO_GSM_TYPE_PLUS_CGACT, (GBoxedCopyFunc) g_object_ref, g_object_unref, "+CGACT");
	_data_->cmd = (FsoGsmPlusCGACT*) _data_->_tmp1_;
	_data_->_tmp2_ = fso_gsm_theModem;
	_data_->_tmp3_ = NULL;
	_data_->_tmp3_ = fso_gsm_simple_at_command_issue ((FsoGsmSimpleAtCommand*) _data_->cmd, GINT_TO_POINTER (0));
	_data_->_tmp4_ = _data_->_tmp3_;
	_data_->_tmp5_ = 0;
	_data_->_state_ = 1;
	fso_gsm_modem_processAtCommandAsync (_data_->_tmp2_, (FsoGsmAtCommand*) _data_->cmd, _data_->_tmp4_, FSO_GSM_MODEM_DEFAULT_RETRIES, fso_gsm_at_pdp_handler_leaveDataState_ready, _data_);
	return FALSE;
	_state_1:
	_data_->_tmp6_ = NULL;
	_data_->_tmp6_ = fso_gsm_modem_processAtCommandAsync_finish (_data_->_tmp2_, _data_->_res_, &_data_->_tmp5_);
...
static gboolean fso_gsm_at_pdp_handler_real_sc_activate_co (FsoGsmAtPdpHandlerScActivateData* _data_) {
	...
	_vala_array_add17 (&_data_->cmdline, &_data_->cmdline_length1, &_data_->_cmdline_size_, _data_->_tmp8_);
	...
	_vala_array_add18 (&_data_->cmdline, &_data_->cmdline_length1, &_data_->_cmdline_size_, _data_->_tmp11_);
	...
	if (_data_->_tmp15_ == NULL) {
		_data_->_tmp16_ = g_error_new_literal (FREE_SMARTPHONE_ERROR, FREE_SMARTPHONE_ERROR_INTERNAL_ERROR, "context parameters not set");
	...
	_data_->_tmp19_ = _data_->_tmp18_->username;
	if (g_strcmp0 (_data_->_tmp19_, "") != 0) {
		_data_->_tmp20_ = _data_->cmdline;
		_data_->_tmp20__length1 = _data_->cmdline_length1;
		_data_->_tmp21_ = g_strdup ("user");
		_vala_array_add19 (&_data_->cmdline, &_data_->cmdline_length1, &_data_->_cmdline_size_, _data_->_tmp21_);
		_data_->_tmp22_ = _data_->cmdline;
		_data_->_tmp22__length1 = _data_->cmdline_length1;
		_data_->_tmp23_ = _data_->data;
		_data_->_tmp24_ = _data_->_tmp23_->contextParams;
		_data_->_tmp25_ = _data_->_tmp24_->username;
		_data_->_tmp26_ = g_strdup (_data_->_tmp25_);
		_vala_array_add20 (&_data_->cmdline, &_data_->cmdline_length1, &_data_->_cmdline_size_, _data_->_tmp26_);
	}
	_data_->_tmp27_ = _data_->data;
	_data_->_tmp28_ = _data_->_tmp27_->contextParams;
	_data_->_tmp29_ = _data_->_tmp28_->password;
	if (g_strcmp0 (_data_->_tmp29_, "") != 0) {
		_data_->_tmp30_ = _data_->cmdline;
		_data_->_tmp30__length1 = _data_->cmdline_length1;
		_data_->_tmp31_ = g_strdup ("password");
		_vala_array_add21 (&_data_->cmdline, &_data_->cmdline_length1, &_data_->_cmdline_size_, _data_->_tmp31_);
		_data_->_tmp32_ = _data_->cmdline;
		_data_->_tmp32__length1 = _data_->cmdline_length1;
		_data_->_tmp33_ = _data_->data;
		_data_->_tmp34_ = _data_->_tmp33_->contextParams;
		_data_->_tmp35_ = _data_->_tmp34_->password;
		_data_->_tmp36_ = g_strdup (_data_->_tmp35_);
		_vala_array_add22 (&_data_->cmdline, &_data_->cmdline_length1, &_data_->_cmdline_size_, _data_->_tmp36_);
	}
	_data_->_tmp37_ = _data_->cmdline;
	_data_->_tmp37__length1 = _data_->cmdline_length1;
	_data_->_tmp38_ = g_strdup ("plugin");
	_vala_array_add23 (&_data_->cmdline, &_data_->cmdline_length1, &_data_->_cmdline_size_, _data_->_tmp38_);
	_data_->_tmp39_ = _data_->cmdline;
	_data_->_tmp39__length1 = _data_->cmdline_length1;
	_data_->_tmp40_ = NULL;
	_data_->_tmp40_ = g_strdup_printf ("%s/ppp2fsogsmd.so", PACKAGE_LIBDIR);
	_vala_array_add24 (&_data_->cmdline, &_data_->cmdline_length1, &_data_->_cmdline_size_, _data_->_tmp40_);
	_data_->_tmp41_ = ((FsoFrameworkAbstractObject*) _data_->self)->logger;
	_data_->_tmp42_ = _data_->cmdline;
	_data_->_tmp42__length1 = _data_->cmdline_length1;
	_data_->_tmp43_ = NULL;
	_data_->_tmp43_ = fso_framework_string_handling_stringListToString (_data_->_tmp42_, _data_->_tmp42__length1);
	_data_->_tmp44_ = _data_->_tmp43_;
	_data_->_tmp45_ = NULL;
	_data_->_tmp45_ = string_to_string (_data_->_tmp44_);
	_data_->_tmp46_ = NULL;
	_data_->_tmp46_ = g_strconcat ("Launching ppp helper with commandline ", _data_->_tmp45_, NULL);
	_data_->_tmp47_ = _data_->_tmp46_;
	_data_->_tmp48_ = FALSE;
	_data_->_tmp48_ = fso_framework_logger_debug (_data_->_tmp41_, _data_->_tmp47_);
	g_assert (_data_->_tmp48_);
	_g_free0 (_data_->_tmp47_);
	_g_free0 (_data_->_tmp44_);
	_data_->_tmp49_ = _data_->cmdline;
	_data_->_tmp49__length1 = _data_->cmdline_length1;
	_data_->_tmp50_ = FALSE;
	_data_->_tmp50_ = fso_gsm_at_pdp_handler_launchPppDaemon (_data_->self, _data_->_tmp49_, _data_->_tmp49__length1);
	if (!_data_->_tmp50_) {
		_g_object_unref0 (_data_->self->ppp);
		_data_->self->ppp = NULL;
		_data_->_tmp51_ = ((FsoFrameworkAbstractObject*) _data_->self)->logger;
		fso_framework_logger_warning (_data_->_tmp51_, "Could not launch PPP helper");
		_data_->_tmp52_ = g_error_new_literal (FREE_SMARTPHONE_ERROR, FREE_SMARTPHONE_ERROR_SYSTEM_ERROR, "Could not launch PPP helper");
		...
		_data_->_tmp53_ = _data_->self->ppp;
		g_signal_connect_object ((FsoFrameworkIProcessGuard*) _data_->_tmp53_, "stopped", (GCallback) _fso_gsm_at_pdp_handler_onPppStopped_fso_framework_iprocess_guard_stopped, _data_->self, 0);
	}
	_data_->_state_ = 1;
	...
	if (_data_->_tmp54_ == NULL) {
		...
		fso_framework_logger_warning (_data_->_tmp55_, "PPP quit immediately; check options and permissions.");
		_data_->_tmp56_ = g_error_new_literal (FREE_SMARTPHONE_ERROR, FREE_SMARTPHONE_ERROR_SYSTEM_ERROR, "PPP helper quit immediately");
