/* rfs.c generated by valac 0.16.0, the Vala compiler
...
static void __lambda7_ (SamsungRfsChannel* self, const gchar* message);
static void ___lambda7__samsung_ipc_log_handler_cb (const gchar* message, gpointer self);
...
SamsungRfsChannel* samsung_rfs_channel_construct (GType object_type, FsoGsmModem* modem, const gchar* name, FsoFrameworkTransport* transport) {
	...
	self->priv->modem = _tmp3_;
	_tmp4_ = name;
	samsung_rfs_channel_set_name (self, _tmp4_);
	_tmp5_ = fso_framework_wakelock_new ("fsogsmd-modem-samsung-rfs");
	_g_object_unref0 (self->priv->wakelock);
	self->priv->wakelock = _tmp5_;
	_tmp6_ = modem;
	_tmp7_ = name;
	fso_gsm_modem_registerChannel (_tmp6_, _tmp7_, (FsoGsmChannel*) self);
	_tmp8_ = ipc_client_new (IPC_CLIENT_TYPE_RFS);
	_ipc_client_free0 (self->priv->rfsclient);
	self->priv->rfsclient = _tmp8_;
	_tmp9_ = self->priv->rfsclient;
	ipc_client_set_log_handler (_tmp9_, ___lambda7__samsung_ipc_log_handler_cb, self);
	_tmp10_ = self->priv->rfsclient;
	ipc_client_set_io_handlers (_tmp10_, _samsung_rfs_channel_modem_read_request_samsung_ipc_transport_cb, self, _samsung_rfs_channel_modem_write_request_samsung_ipc_transport_cb, self);
	return self;
}


SamsungRfsChannel* samsung_rfs_channel_new (FsoGsmModem* modem, const gchar* name, FsoFrameworkTransport* transport) {
	return samsung_rfs_channel_construct (SAMSUNG_TYPE_RFS_CHANNEL, modem, name, transport);
}
