...
static gboolean __lambda2_ (SamsungIpcChannel* self);
...
static void __lambda3_ (SamsungIpcChannel* self, const gchar* message);
static void ___lambda3__samsung_ipc_log_handler_cb (const gchar* message, gpointer self);
...
SamsungIpcChannel* samsung_ipc_channel_construct (GType object_type, const gchar* name, FsoFrameworkTransport* transport) {
	...
	FsoFrameworkTransport* _tmp0_;
	const gchar* _tmp1_;
	SamsungUnsolicitedResponseHandler* _tmp2_;
	SamsungUnsolicitedResponseHandler* _tmp3_;
	FsoFrameworkWakelock* _tmp4_;
	FsoGsmModem* _tmp5_;
	const gchar* _tmp6_;
	FsoGsmModem* _tmp7_;
	struct ipc_client* _tmp8_;
	struct ipc_client* _tmp9_;
	struct ipc_client* _tmp10_;
	g_return_val_if_fail (name != NULL, NULL);
	_tmp0_ = transport;
	self = (SamsungIpcChannel*) fso_framework_abstract_command_queue_construct (object_type, _tmp0_);
	_tmp1_ = name;
	samsung_ipc_channel_set_name (self, _tmp1_);
	_tmp2_ = samsung_unsolicited_response_handler_new ();
	_tmp3_ = _tmp2_;
	samsung_ipc_channel_set_urchandler (self, _tmp3_);
	_g_object_unref0 (_tmp3_);
	_tmp4_ = fso_framework_wakelock_new ("fsogsmd-modem-samsung");
	_g_object_unref0 (self->priv->wakelock);
	self->priv->wakelock = _tmp4_;
	...
	g_signal_connect_object (_tmp7_, "signal-status-changed", (GCallback) _samsung_ipc_channel_onModemStatusChanged_fso_gsm_modem_signal_status_changed, self, 0);
	_tmp8_ = ipc_client_new (IPC_CLIENT_TYPE_FMT);
	_ipc_client_free0 (self->priv->fmtclient);
	self->priv->fmtclient = _tmp8_;
	_tmp9_ = self->priv->fmtclient;
	ipc_client_set_log_handler (_tmp9_, ___lambda3__samsung_ipc_log_handler_cb, self);
	_tmp10_ = self->priv->fmtclient;
	ipc_client_set_io_handlers (_tmp10_, _samsung_ipc_channel_modem_read_request_samsung_ipc_transport_cb, self, _samsung_ipc_channel_modem_write_request_samsung_ipc_transport_cb, self);
	return self;
}


SamsungIpcChannel* samsung_ipc_channel_new (const gchar* name, FsoFrameworkTransport* transport) {
	return samsung_ipc_channel_construct (SAMSUNG_TYPE_IPC_CHANNEL, name, transport);
}
...
void samsung_ipc_channel_enqueue_async (SamsungIpcChannel* self, int type, unsigned short command, guint8* data, int data_length1, gint retry, gint timeout, GAsyncReadyCallback _callback_, gpointer _user_data_) {
	...
	_data_->data = _tmp4_;
	_data_->data_length1 = _tmp4__length1;
	_tmp5_ = retry;
	_data_->retry = _tmp5_;
	_tmp6_ = timeout;
...
static gboolean samsung_ipc_channel_enqueue_async_co (SamsungIpcChannelEnqueueAsyncData* _data_) {
	...
	_data_->_tmp29_ = (_data_->_tmp28_ != NULL) ? _vala_array_dup2 (_data_->_tmp28_, _data_->_tmp28__length1) : ((gpointer) _data_->_tmp28_);
...
static gboolean samsung_ipc_channel_real_suspend_co (SamsungIpcChannelSuspendData* _data_) {
	...
	g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) 100, ___lambda4__gsource_func, block2_data_ref (_data_->_data2_), block2_data_unref);
	_data_->_state_ = 1;
	return FALSE;
	_state_1:
	;
